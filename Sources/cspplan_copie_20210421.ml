      ...
      List.iter (fun ((f : 'fluent),lev) ->
        ...
        Array.iter (fun (a : 'action) ->
          ...
          begin
            ...
            if f#maxlevel < i then begin f#set_maxlevel i; newsubgoals := (p,i) :: !newsubgoals; end
            ) a#prec
          end
        ) f#producers
      ) !subgoals;
...
(* let allactions = Array.mapi (fun i f ->
                    let noop = (self#create_action ("NOOP-"^f#to_string^"__"^string_of_int(!current_level)) [| |] 1. 0 [|f,f#atom#timedata|] [| |] [|f,f#atom#timedata|] [| |]) in
                    noop#set_num (nb_actions + i);
                    noop
                  ) pdata#fluents in *)


Array.iter (fun a -> Printf.printf "%d: %s [%d,%d]\n" a#num a#to_string a#level a#maxlevel) pdata#actions;
(* Array.iter (fun a -> Printf.printf "%d: %s [%d,%d]\n" a#num a#to_string a#level a#maxlevel) allactions; *)


Printf.printf "<instance>
<presentation format=\"XCSP 2.1\" type=\"CSP\" name=\"%s--%s\">
</presentation>\"\n" pdata#domain_name pdata#problem_name;



let current_level = ref rpg_max_level in
while !current_level > 0 do begin
Array.iter (fun f -> 
              let fprods = Array.fold_left (fun l a ->  if (a#level <= !current_level) && (!current_level <= a#maxlevel) then a::l else l) [] f#producers in
             (* if (f#level <= !current_level - 1) && (!current_level - 1 <= f#maxlevel) then
                let noop =  (self#create_action ("NOOP-"^f#to_string^"__"^string_of_int(!current_level)) [| |] 1. 0 [|f,f#atom#timedata|] [| |] [|f,f#atom#timedata|] [| |]) in
                if (List.length fprods) == 0 then lvars := (f,!current_level,[noop])::!lvars
                else lvars := (f,!current_level,noop::fprods)::!lvars
              else *) if (List.length fprods) != 0 then lvars := (f,!current_level,fprods)::!lvars
) pdata#fluents;
...
(*
Array.iter (fun f -> lvars := (f,rpg_max_level)::!lvars) pdata#goal;

let current_level = ref (rpg_max_level - 1) in
while !current_level > 0 do begin
Array.iter (fun f -> 
          if (Array.fold_left (fun l a -> 
            if (a#level <= !current_level) && (!current_level <= a#maxlevel) then l+1 else l
          ) 0 f#producers) != 0 then lvars := (f,!current_level)::!lvars
) pdata#fluents;
current_level := !current_level - 1;
...
List.iter (fun (f,level,fprods) -> 
          (*let fprods = Array.fold_left (fun l a -> 
            if (a#level <= level) && (level <= a#maxlevel) then a::l else l
          ) [] f#producers in *)
          begin
            Printf.printf "<domain name=\"D_%s__%d\" nbValues=\"%d\">\n%s" f#to_string level (1 + List.length fprods) (if level==rpg_max_level then "" else "-1 ");
            List.iter (fun a ->
              Printf.printf "%s " a#to_string (* (nb_actions * level + a#num) *)
            ) fprods;
            Printf.printf "\n</domain>\n"
          end
) !lvars;
...
Array.iter (fun f -> 
          begin
            lvars := (f,rpg_max_level)::!lvars;
            Printf.printf "<domain name=\"D_%s__%d\" nbValues=\"%d\">\n" f#to_string rpg_max_level (Array.length f#producers);
            Array.iter (fun a ->
              if (a#level <= !current_level) && (!current_level <= a#maxlevel) then
              Printf.printf "%d " (* a#to_string *) (nb_actions * rpg_max_level + a#num);
            ) f#producers;
            Printf.printf "\n</domain>\n"
          end
) pdata#goal;
...
Array.iter (fun f -> 
          let fprods = Array.fold_left (fun l a -> 
            if (a#level <= !current_level) && (!current_level <= a#maxlevel) then a::l else l
          ) [] f#producers in
          if (List.length fprods) != 0 then begin
            lvars := (f,!current_level)::!lvars;
            Printf.printf "<domain name=\"D_%s__%d\" nbValues=\"%d\">\n-1 " f#to_string !current_level (1 + List.length fprods);
            List.iter (fun a ->
              Printf.printf "%d " (* a#to_string *) (nb_actions * !current_level + a#num)
            ) fprods;
            Printf.printf "\n</domain>\n"
          end
) pdata#fluents;
...
List.iter (fun (f,level,_) -> Printf.printf "<variable name=\"%s__%d\" domain=\"D_%s__%d\"/>\n" f#to_string level f#to_string level) !lvars;

Printf.printf "</variables>\n";

Printf.printf "<relations nbRelations=\"%d\">\n" 1; (* ajouter le nombre de relations *)

let rec relations l =
  match l with
  (f1,level1,fprods1)::l2 -> begin
                      List.iter (fun (f2,level2,fprods2) -> 
                              if level1==level2 then begin
                                Printf.printf "<relation name=\"relMutex__%s__%d__%s__%d\" arity=\"2\" nbTuples=\"%d\" semantics=\"conflicts\">\n" f1#to_string level1 f2#to_string level2 1;
                                (* List.iter (fun a1 ->
                                  List.iter (fun a2 ->
                              ...
                              else if level1+1==level2 then Printf.printf "<relation name=\"relCausality__%s__%d__%s__%d\" arity=\"2\" nbTuples=\"%d\" semantics=\"conflicts\">\n" f1#to_string level1 f2#to_string level2 1
                              else if level1==level2+1 then Printf.printf "<relation name=\"relCausality__%s__%d__%s__%d\" arity=\"2\" nbTuples=\"%d\" semantics=\"conflicts\">\n" f2#to_string level2 f1#to_string level1 1
                              ) l2;
                      (relations l2) + 1 (* ajuster le nombre de relations *)
                    end
  | [] -> Printf.printf "\n"; 0
  in
...
Printf.printf "<constraints nbConstraints=\"%d\">" 1; (* ajouter le nombre des contraintes *)
